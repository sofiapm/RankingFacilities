module Kpi
	...
	def self.calc_quarter_average results
		if results.size>0
			result_by_quarter=results.in_groups(4, false)
			avg_by_quarter = []
			result_by_quarter.each do |r|
				avg_by_quarter << r.sum / r.size.to_f
			end
		else
			avg_by_quarter=[0,0,0,0]
		end
		avg_by_quarter
	end
	...
	def self.measurement *args
		...
		year=args[2]
		if year
			measures = facility.measures.where("strftime('%Y', start_date) <= ? AND strftime('%Y', start_date) <= ?", year, year).sort_by{|vn| vn[:date]}
		else
			measures = facility.measures.sort_by{|vn| vn[:date]}
		end
		array = []
	...
	def self.static_measurement *args
		...
		year=args[2]
		if year
			measures = FacilityStaticMeasure.where(facility_id: facility.id).where("strftime('%Y', start_date) <= ? AND strftime('%Y', start_date) <= ?", year, year).sort_by{|vn| vn[:date]}
	...
	def self.best_internal_work_cost(facility, year)
		if year == ''
	...
	def self.best_water_consumption_fte(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.best_waste_production_fte(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.best_capacity_vs_utilization(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.best_space_experience(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.best_energy_consumption(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.best_cleaning_cost_nfa(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.best_space_cost_nfa(facility, year)
		if year == ''
			year = Date.current.year
		end
	...
	def self.costs_year facility
		year = Date.current.year
		hash = {:first => {:name => year-1, :values => [avg(cleaning_cost_nfa(facility, (year-1).to_s)[:values]), avg(space_cost_nfa(facility, (year-1).to_s)[:values]), avg(occupancy_cost_nfa(facility, (year-1).to_s)[:values])]}}
		hash[:second]={:name => year, :values => [avg(cleaning_cost_nfa(facility, year.to_s)[:values]), avg(space_cost_nfa(facility, year.to_s)[:values]), avg(occupancy_cost_nfa(facility, year.to_s)[:values]) ]}
		hash[:third]={:name => year+1, :values => [avg(cleaning_cost_nfa(facility, (year+1).to_s)[:values]), avg(space_cost_nfa(facility, (year+1).to_s)[:values]), avg(occupancy_cost_nfa(facility, (year+1).to_s)[:values]) ]}
